...
if os.environ.get("INTEGRATION_TEST", "").lower() == "true":
    project_id = "mock-project"
    class MockLogger:
        def log_struct(self, data, severity="INFO"):
            std_logging.info(f"MOCK LOG STRUCT [{severity}]: {data}")
    logger = MockLogger()
else:
    try:
        ...
        class MockLogger:
            def log_struct(self, data, severity="INFO"):
                std_logging.info(f"FALLBACK LOG STRUCT [{severity}]: {data}")
        logger = MockLogger()
allow_origins = (
...
app.title = "sec-incident-agent"
app.description = "API for interacting with the Agent sec-incident-agent"
